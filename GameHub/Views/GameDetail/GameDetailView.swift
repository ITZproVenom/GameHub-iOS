...
struct GameDetailView: View {
    ...
    var body: some View {
        ...
        .fullScreenCover(isPresented: $viewModel.isPlaying) {
            MetalPlayView {
        ...
        sectionCard(title: "Windows Container") {
            if let container = viewModel.container {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    HStack(spacing: 12) {
                        Button { viewModel.resetContainer() } label: {
                            Label("Reset", systemImage: "arrow.counterclockwise")
                        }
                        .buttonStyle(.bordered)
                        .disabled(container.status == .resetting)
                        Button(role: .destructive) { viewModel.deleteContainer() } label: {
                    ...
                    NavigationLink { containerDetailView(with: containerVM) } label: {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    Label("No container for this game yet.", systemImage: "shippingbox")
                        .font(.subheadline)
                    Button { viewModel.createContainer() } label: {
    ...
    private func infoRow(_ title: String, value: String) -> some View {
        HStack(alignment: .top) {
            ...
            Spacer()
            Text(value).multilineTextAlignment(.trailing).lineLimit(3)
        }
        .font(.footnote)
    }
...
struct ContainerDetailView: View {
    ...
    private var dangerSection: some View {
        Section("Danger Zone") {
            Button(role: .destructive) { viewModel.resetContainer() } label: {
